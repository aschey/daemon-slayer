...
pub(crate) fn define_service(ident: Ident, crate_name: proc_macro2::TokenStream) -> TokenStream {
    quote! {
        ...
        static __FINISHED_TX: std::sync::OnceLock<#crate_name::tokio::sync::mpsc::Sender<std::result::Result<(), #crate_name::ServiceError<<#ident as #crate_name::Handler>::Error>>>> = 
            std::sync::OnceLock::new();
        ...
        impl #crate_name::Service for #ident {
            async fn run_as_service(input_data: Option<Self::InputData>) -> std::result::Result<(), #crate_name::ServiceError<Self::Error>> {
