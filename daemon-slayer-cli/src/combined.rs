    ...
    H: daemon_slayer_server::Service + daemon_slayer_server::Handler ,
{
    pub fn builder(manager: ServiceManager) -> builder::CliBuilder<H> {
        ...
        builder::CliBuilder::from_manager(manager, commands, clap::Command::default())
    }
    ...
    pub(crate) fn from_builder(builder: builder::CliBuilder<H>) -> Self {
        ...
        Self {
            client_cli: client::ClientCli::builder(manager).with_base_command(builder.base_cmd.clone()).build(),
            server_cli: server::ServerCli::builder(builder.display_name, builder.description).with_base_command(builder.base_cmd.clone()).build(),
            commands,
            display_name,
            description,
    ...
    pub(crate) fn build_cmd(
        &self
         ...
         cmd = cmd.subcommand(clap::Command::new("testcmd"));
         cmd
     }
}
    ...
    H: daemon_slayer_server::Service + daemon_slayer_server::Handler,
{
    async fn handle_input(self) -> Result<InputState, Box<dyn Error>> {
        let cmd = self.build_cmd();//util::build_cmd(self.base_command.clone(), &self.display_name, &self.description, &self.commands);
