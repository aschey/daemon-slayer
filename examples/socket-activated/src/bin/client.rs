...
async fn run(offset_time: OffsetTime<Rfc3339>) -> Result<(), BoxedError> {
    ...
    let manager = client::builder(
        ...
        current_exe()?
            ...
            .join("socket-activated-server")
            .try_into()?,
    )
    .with_description("test service")
    .with_arg(&socket_activated::run_argument())
    ...
    .with_activation_sockets(socket_activated::sockets())
    .build()
    .await?;

    let logger_builder = LoggerBuilder::new(socket_activated::label(), offset_time);
